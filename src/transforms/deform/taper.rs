...
impl Transform for Taper {
    fn apply(&self, model: &mut Model) -> Result<()> {
        ...
        for vertex in &mut model.mesh.vertices {
            ...
            if self.axis.x.abs() > 0.9 {
                // X is the main axis
                scaled_normal_perp1 = if scale_y != 0.0 { normal_perp1 / scale_y } else { normal_perp1 };
                scaled_normal_perp2 = if scale_z != 0.0 { normal_perp2 / scale_z } else { normal_perp2 };
            } else if self.axis.y.abs() > 0.9 {
                // Y is the main axis
                scaled_normal_perp1 = if scale_x != 0.0 { normal_perp1 / scale_x } else { normal_perp1 };
                scaled_normal_perp2 = if scale_z != 0.0 { normal_perp2 / scale_z } else { normal_perp2 };
            } else {
                // Z is the main axis
                scaled_normal_perp1 = if scale_x != 0.0 { normal_perp1 / scale_x } else { normal_perp1 };
                scaled_normal_perp2 = if scale_y != 0.0 { normal_perp2 / scale_y } else { normal_perp2 };
            }
