...
fn calculate_normal(v0: nalgebra::Point3<f32>, v1: nalgebra::Point3<f32>, v2: nalgebra::Point3<f32>) -> Vector3<f32> {
    let edge1 = v1 - v0;
    let edge2 = v2 - v0;
...
fn export_binary_stl<P: AsRef<Path>>(model: &Model, path: P) -> Result<()> {
    ...
    // Header (80 bytes)
    let header = format!("Binary STL generated by model-generator - Model: {}", model.name);
