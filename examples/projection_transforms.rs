...
use model_generator::transforms::projection::{Perspective, Orthographic, Cylindrical};
use model_generator::transforms::basic::Translate;
use nalgebra::{Point3, Vector3};
use model_generator::{Model, Vertex, Face};
...
fn main() -> model_generator::Result<()> {
    ...
    let mut sphere = Sphere::new().radius(0.25).center(0.5, 0.0, 0.5).segments(16).rings(8).build();
...
fn create_grid(width: f32, height: f32, columns: usize, rows: usize) -> Model {
    ...
    for i in 0..rows {
        for j in 0..columns {
            ...
            model.mesh.add_face(Face::triangle(
                vertices[top_left],
