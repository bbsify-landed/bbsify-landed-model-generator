...
use model_generator::transforms::{Scale, Translate, Rotate};
...
fn test_transform_plugin() {
    ...
    let scale_plugin = TransformPlugin::new(
        "scale_double",
        "Doubles the size",
        Scale::uniform(2.0),
    );
...
fn test_custom_plugin() {
    ...
    let original_z: Vec<f32> = model.mesh.vertices.iter()
        .map(|v| v.position.z)
        .collect();
...
fn test_plugin_error_handling() {
    ...
    impl Plugin for FailingPlugin {
        ...
        fn process(&self, _model: &mut Model) -> Result<()> {
            Err(model_generator::Error::PluginError("Intentional failure".to_string()))
        }
    }
