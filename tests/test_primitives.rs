...
fn test_cube_creation() {
    ...
    assert!(custom_cube.mesh.vertices.iter().all(|v| v.tex_coords.is_none()));
}
...
fn test_sphere_creation() {
    ...
    let avg_x: f32 = custom_sphere.mesh.vertices.iter().map(|v| v.position.x).sum::<f32>() / 
                    custom_sphere.mesh.vertices.len() as f32;
    let avg_y: f32 = custom_sphere.mesh.vertices.iter().map(|v| v.position.y).sum::<f32>() / 
                    custom_sphere.mesh.vertices.len() as f32;
    let avg_z: f32 = custom_sphere.mesh.vertices.iter().map(|v| v.position.z).sum::<f32>() / 
                    custom_sphere.mesh.vertices.len() as f32;
...
fn test_cylinder_creation() {
    ...
    // Verify height
    let max_y = custom_cylinder.mesh.vertices.iter().map(|v| v.position.y).fold(f32::NEG_INFINITY, f32::max);
    let min_y = custom_cylinder.mesh.vertices.iter().map(|v| v.position.y).fold(f32::INFINITY, f32::min);
